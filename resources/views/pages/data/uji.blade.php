<x-app-layout>
        ...
        <div class="w-full mx-auto sm:px-6 lg:px-4">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    {{ __('This is the Data Uji page.') }}

                    <div class="py-2">
                        <div class="max-w-full sm:px-6 lg:px-8">
                            {{-- Form upload CSV --}}
                            <form action="{{ route('data_uji.upload') }}" method="POST" enctype="multipart/form-data"
                                class="mt-4 inline-flex gap-4">
                                @csrf
                                <input type="file" name="file"
                                    class="border p-2 rounded dark:bg-gray-700 dark:text-gray-300" required>
                                <button type="submit"
                                    class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Upload
                                    File</button>
                            </form>

                            {{-- Tombol Tambah Data --}}
                            <button onclick="openModal()"
                                class="bg-green-500 text-white px-4 py-2 rounded mt-4 inline-flex items-center hover:bg-green-600">
                                Tambah Data
                            </button>
                            <div id="addDataModal"
                                class="fixed inset-0 bg-gray-500 bg-opacity-50 hidden justify-center items-center z-50">
                                <div
                                    class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-1/3 transform -translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2">
                                    <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">Tambah Data
                                        Latih</h3>
                                    <form action="{{ route('add-data.store') }}" method="POST">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <textarea id="content" name="content" class="w-full p-2 border rounded dark:bg-gray-700 dark:text-gray-300" required>{{ old('content') }}</textarea>
                                        </div>
                                        <div class="mb-4">
                                            <label for="sentimen"
                                    ...
                                    <tbody>
                                        @foreach ($dataUji as $data)
                                            <tr
                                                ...
                                                <td
                                                    class="px-6 py-4 whitespace-normal text-sm font-medium text-gray-800 break-words dark:text-gray-100">
                                                    {{ $data->content }}
                                                </td>
                                                <td
                                                    class="px-6 py-4 whitespace-normal text-sm text-gray-800 break-words dark:text-gray-100">
                                                    {{ $data->sentimen }}
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                                    ...
                                                    <!-- Tombol Delete -->
                                                    <form action="{{ route('data_uji.destroy', $data->id) }}"
                                                        method="POST" style="display:inline;"
                                                        onsubmit="return confirm('Are you sure you want to delete this data?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-red-600 hover:text-red-800 focus:outline-none focus:text-red-800 dark:text-red-400 dark:hover:text-red-500">
                                                            Delete
                                                        </button>
                                                    </form>
                                                </td>
    ...
    <div id="editDataModal" class="fixed inset-0 bg-gray-500 bg-opacity-50 hidden justify-center items-center z-50">
        <div
            class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-1/3 transform -translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2">
            <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">Edit Data Latih</h3>
            <form action="{{ route('updateuji-data') }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    <label for="editContent"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Content</label>
                    <textarea id="editContent" name="content" class="w-full p-2 border rounded dark:bg-gray-700 dark:text-gray-300"
                        required></textarea>
                </div>
                <div class="mb-4">
                    <label for="editSentiment"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Sentiment</label>
                    <select id="editSentiment" name="sentiment"
                        class="w-full p-2 border rounded dark:bg-gray-700 dark:text-gray-300" required>
